...
def _solve_kwargs(config) -> dict[str, Any]:
    """``ResidualSufficientStatistics.solve`` options read off either config type.

    ``DirectResidualConfig`` has no ``ridge_mode``/``ridge_absolute``; for it the
    defaults reproduce the historical trace-normalized solve exactly, while a
    ``ResidualCompletionConfig`` with ``ridge_mode='absolute'`` is honoured on
    every fit path rather than only the per-position ones.
    """
    return {
        "ridge_relative": config.ridge_relative,
        "ridge_estimator": config.ridge_estimator,
        "exact_form": config.exact_form,
        "ridge_mode": getattr(config, "ridge_mode", "trace_normalized"),
        "ridge_absolute": getattr(config, "ridge_absolute", None),
    }
...
def iter_capture_tokens(
    model,
    batches,
    requests: Mapping[str, tuple[int, str]],
    device,
    *,
    family_adapter=None,
    store_device="cpu",
    ...
    ``mask_padding`` packs each batch to its real tokens, as in ``capture_tokens``.
    """
    ...
    try:
        ...
        for batch in batches:
            ...
            batch_mask = layout.token_mask(batch) if mask_padding else None
            values: dict[str, list[torch.Tensor]] = {key: [] for key in requests}

            def store(name, tensor, *, _batch_size=batch_size, _mask=batch_mask, _values=values):
                if isinstance(tensor, tuple):
                    tensor = tensor[0]
                tokens = _to_tokens(tensor.detach(), batch_size=_batch_size)
                if _mask is not None:
                    if tuple(_mask.shape) != tuple(tokens.shape[:2]):
                        raise RuntimeError(
                            f"Padding mask {tuple(_mask.shape)} does not match captured tokens "
                            f"{tuple(tokens.shape[:2])}"
                        )
                    tokens = tokens[_mask.to(tokens.device)].unsqueeze(0)
                if store_device == "cpu":
...
def capture_tokens(model, batches, requests: Mapping[str, tuple[int, str]], device, *, family_adapter=None, mask_padding=False):
    ...
    for batch_values in iter_capture_tokens(
        model, batches, requests, device, family_adapter=family_adapter, mask_padding=mask_padding
    ):
...
def _fit_all_positions_independent(
    ...
    *caller* side. One ``"out"`` (boundary) request is registered per
    ...
    captured = capture_tokens(
        target_model, batches, requests, device, family_adapter=family_adapter, mask_padding=mask_padding
    )
...
def _finalize_independent_component(
    ...
    # Relative to the target's own output energy; base_sq=None falls back to
    # the historical absolute check against the desired energy.
    tolerance = (
        1e-12 * max(desired_sq, 1.0) if base_sq is None else _PRISTINE_EFFECT_RTOL * max(base_sq, 1.0)
    )
    if effect_sq > tolerance:
        raise RuntimeError(
            "Direct completion started from a target model that is not the native "
            f"base: nonzero pre-fit effect at position {pos} (||T-T0||^2={effect_sq:.3e})"
        )
    correction, diag = stats.solve(**_solve_kwargs(config))
    correction = correction.cpu()
    diag["bias_correction"] = diag["bias_correction"].cpu()
