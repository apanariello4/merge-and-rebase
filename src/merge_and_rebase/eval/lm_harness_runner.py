...
# Task definitions this repo ships. `harmbench_refusal` lives here because
# lm-eval has no refusal benchmark, and refusal is the only thing an
# abliteration task vector actually changes. The YAMLs are generated by
# scripts/setup_harmbench_task.py -- if the directory holds no YAML the task
# simply will not resolve, which is why the path is probed rather than assumed.
EXTRA_TASK_DIRS = Path(__file__).resolve().parent / "harness_tasks"
...
def run(
    ...
    # Tasks this repo defines itself (currently harmbench_refusal) live beside
    # the built-ins rather than replacing them, so include_defaults stays on.
    # TaskManager is imported separately from simple_evaluate: the ImportError
    # above means "lm-eval is not installed", and a TaskManager that has moved in
    # a future lm-eval must not be reported as that. Losing it costs only the
    # in-repo tasks, so it degrades to None rather than failing the run.
    task_manager = None
    if EXTRA_TASK_DIRS.is_dir():
        try:
            from lm_eval.tasks import TaskManager

            task_manager = TaskManager(include_path=str(EXTRA_TASK_DIRS))
        except Exception:
            task_manager = None
